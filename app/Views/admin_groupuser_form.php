    ...
    <div class="col-lg-12">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <?php 
            $was_validated = "";
            if($validation->hasError('group_name')) {
          ...
          <form action="<?=$action?>" class="row g-3 needs-validation <?=$was_validated?>" method="POST" novalidate>
            ...
            <div class="col-12">
              <label for="inputNanme4" class="form-label">Grup Pengguna</label>
              <div class="input-group has-validation">
                <input type="text" name="group_name" class="form-control" value="<?=$group_name?>"  <?=($validation->hasError('group_name')?'required=""':'')?>>
                <?php if($validation->hasError('group_name')) {?>
                  <div class="invalid-feedback">
                        <?= $validation->getError('group_name')?> 
                  </div>
                <?php }?>
